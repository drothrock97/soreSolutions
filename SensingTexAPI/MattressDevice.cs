...
namespace SensingTexAPI
{
	...
	public class DataReadyEventArgs : EventArgs
	{
		...
		private double[,] compensationAlgorithm(double[,] inputData)
		{
			int i = 0;
			int j = 0;
			double compensationCoefficient;
			int numSensorsActivated = 0;
			double THRESHOLD_VALUE = 100;
			double[,] outputData = new double[16, 16];

			//copying input array to output array
			for (i = 0; i < 16; i++)
				for (j = 0; j < 16; j++)
					outputData[i, j] = inputData[i, j];
			//Applying algorithm to each column
			for (j = 0; j < 16; j++)
			{
				compensationCoefficient = 4095;
				numSensorsActivated = 0;
				//iterate all sensors in j column
				//count how many sensor points are activated and calculate the compensationCoeffient
				for (i = 0; i < 16; i++)
				{
					//only the values above threshold considered valid
					if (outputData[i, j] > THRESHOLD_VALUE)
					{
						compensationCoefficient -= outputData[i, j];
				...
				//If there is more than one sensor point activated, algorithm will be applied
				if (numSensorsActivated >= 2)
				{
					//apply the algorithm to all sensor points in j column
					for (i = 0; i < 16; i++)
					{
						...
						{
							//APPLYING ALGORITHM
							outputData[i, j] = 4095 * outputData[i, j] / (4095 + outputData[i, j] - compensationCoefficient);
							outputData[i, j] = Math.Round(outputData[i, j], 0);
